return {
  ...
  },
  {
    ...
    opts = {
      ...
      servers = {
        -- Disable competing type checkers to prevent duplicate diagnostics
        pyright = { enabled = false },
        basedpyright = { enabled = false },
        -- Enable ty for type checking, definition jumps, and auto-imports
        ty = { enabled = true, autostart = true },
        -- Keep ruff running concurrently for instant linting and formatting
        ruff = {
          enabled = true,
          autostart = true,
          cmd = { "ruff", "server" },
          -- This ensures Ruff scans properly for your local configuration file
  ...
  },
  {
    ...
    opts = function(_, opts)
      table.insert(opts.sections.lualine_x, 1, {
        function()
          return require("config.runner").status()
        end,
      })
    end,
  },
